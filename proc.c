...
int
fork(numeroTickets) // el entero que creamos y definimos antes
{
  int i, pid;
  struct proc *np;
  ...
  *np->tf = *cp->tf;
  ...
  np->state = RUNNABLE;
    if(numeroTickets==0){
        np->numeroTickets=100; // 100 siendo la cte que definimos
    }else{
        np->numerotickets=numerotickets; // con esto le asignamos tickets a los procesos RUNNABLE
    }
...
userinit(void)
{
  ...
  p->tf->eflags = FL_IF;
  p->tf->esp = p->sz;
  p->numeroTickets = 100 // el número 100 puede ser reemplazado por cualquiera que queramos de cte.
...
procdump(void)
{
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    ...
    if(p->state >= 0 && p->state < NELEM(states) && states[p->state])
      ...
      state = "???";
    cprintf("%d %s %s %d = %d", p->pid, state, p->name, p->numeroTickets, p->win);
    if(p->state == SLEEPING){
